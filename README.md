...
- 电脑和手机在同一局域网时，可通过 Web/PWA 前端查看并控制多台路灯设备。
...
当前项目只提供同一局域网内的移动端访问，不部署公网服务，也不把 ESP32 摄像头视频流直接暴露到互联网。该方式已满足原型联调和竞赛现场演示，同时避免开发服务器、摄像头入口及控制接口未经认证直接暴露。
...
- React Web/PWA 前端可实时展示状态、控制亮度、查看故障、打开摄像头入口和按设备启动/停止 YOLO 服务。
- 两台设备可分别运行独立 YOLO 检测进程，识别结果发布到各自的 `vision` 主题。
- 手机可通过电脑局域网地址访问前端并控制设备。

下一步建议：

- 同步更新竞赛作品创意表和设计文档，明确双节点、多路 YOLO、批量控制和手机局域网控制已经完成。
...
- 不将公网部署、原生 APP、MySQL、GPS 实时定位或物业工单联动描述为已经实现的功能。
